...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("Delete")]
        public async Task<IActionResult> Delete([FromBody] string DevicesName)
        {
            var device = _devices.GetDeviceByName(DevicesName).Result;
            if (device is null) 
            {
                return StatusCode(400, "Такого устройства нет");
            }
            else
            {
                _devices.DeleteDevice(device);
            }
            return StatusCode(200, $"{device.Name} удален");
        }
